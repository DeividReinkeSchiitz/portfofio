...
function WebStacks(stacks: Stack[]) {
    ...
    for (let i = 0; i < stacks.length; i++) {
        cur_stacks.push(
            <div key={i} className={`hover:shadow-lg hover:scale-105 font-normal hover:transform transition-transform duration-300 ease-in-out flex p-3`}>
    ...
    return (
        <div className={`ml-6 grid grid-cols-2 gap-x-2 gap-y-2 mt-5 items-center justify-center`}>
            {cur_stacks}
        </div>
    )
}
